# Notes Directory

This folder contains study notes, interview preparation material, and topic-specific learning guides.

## Main Study Areas

### Core Engineering
- [Java](./JAVA/README.md)
- [Linux](<./Linux Notes/README.md>)
- [Git](<./GIT Notes/README.md>)
- [REST API](<./REST-API Notes/README.md>)
- [Spring Boot and Java Backend](<./SPRINGBOOT-JAVA-BACKEND Notes/README.md>)
- [Microservices and System Design](<./MICROSERVICES-SYSTEM-DESIGN Notes/README.md>)
- [Maven, SonarQube, Testing](<./MAVEN-SONARQUBE-TESTING Notes/README.md>)

### Platform and DevOps
- [Docker](<./DOCKER Notes/README.md>)
- [DevOps](<./DEVOPS Notes/README.md>)
- [AWS](<./AWS Notes/README.md>)
- [Jenkins, Terraform, Terragrunt](<./JENKINS-TERRAFORM-TERRAGRUNT Notes/README.md>)
- [ArgoCD and Kubernetes](<./ARGOCD-KUBERNETES Notes/README.md>)
- [Kubernetes Deep](<./KUBERNETES-DEEP Notes/README.md>)
- [GitOps, Helm, External Secrets](<./GITOPS-HELM-EXTERNALSECRETS Notes/README.md>)

## Interview Support
- [Master interview notes index](./master-interview-notes-index.md)
- [HR interview questions and answers](./hr-interview-questions-and-answers.md)
- [Project explanation answers for interviews](./project-explanation-answers-for-interviews.md)
- [Scenario-based mock interview rounds](./scenario-based-mock-interview-rounds.md)
- [5 to 7 years mock interview rounds with ideal answers](./5-to-7-years-mock-interview-rounds-with-ideal-answers.md)
- [Senior Java backend interview roadmap: 5 to 7 years](./JAVA/interview-guides/java-senior-backend-5-to-7-years-roadmap.md)

## Recommended Study Path

1. Start with Java, Linux, Git, REST API, and Spring Boot basics.
2. Move to Docker, Kubernetes, ArgoCD, GitOps, and AWS.
3. Revise database, testing, microservices, and system design material.
4. Finish with interview support files and mock rounds.
