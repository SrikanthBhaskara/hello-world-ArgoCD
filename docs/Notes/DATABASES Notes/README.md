# Databases Notes

## Structure

- [Database interview index](./database-interview-index.md)
- [Database interview questions with short and better answers](./database-interview-questions-with-short-and-better-answers.md)
- [Database real-time interview questions: 0 to 7 years](./database-real-time-interview-questions-0-to-7-years.md)
- [Database real-time query and scenario questions: 0 to 7 years](./database-real-time-query-scenarios-0-to-7-years.md)
- [Database senior 5 to 7 years production interview guide](./database-senior-5-to-7-years-production-interview-guide.md)

### SQL

- [SQL and RDBMS deep notes](./SQL/sql-rdbms-deep-notes.md)
- [PostgreSQL deep notes](./SQL/postgresql-deep-notes.md)
- [Database performance, scale, and reliability deep notes](./SQL/database-performance-scale-and-reliability-deep-notes.md)
- [SQL query examples and interview problems](./SQL/sql-query-examples-and-interview-problems.md)

### NoSQL

- [MongoDB deep notes](./NOSQL/mongodb-deep-notes.md)
- [MongoDB query examples and interview problems](./NOSQL/mongodb-query-examples-and-interview-problems.md)
- [Cassandra deep notes](./NOSQL/cassandra-deep-notes.md)
- [Cassandra CQL query patterns and interview notes](./NOSQL/cassandra-cql-query-patterns-and-interview-notes.md)
- [Redis deep notes](./NOSQL/redis-deep-notes.md)
- [Redis interview questions with short and better answers](./NOSQL/redis-interview-questions-with-short-and-better-answers.md)
- [Redis troubleshooting scenarios with ideal answers](./NOSQL/redis-troubleshooting-scenarios-with-ideal-answers.md)

### Search

- [Solr deep notes](./SEARCH/solr-deep-notes.md)
- [Solr query examples and search interview notes](./SEARCH/solr-query-examples-and-search-interview-notes.md)

## Best Use

1. Start with SQL and RDBMS fundamentals.
2. Learn PostgreSQL for relational and production-focused interview coverage.
3. Use the database performance note for execution plans, partitioning, replication, pooling, and migration safety.
4. Practice SQL query problems.
5. Move to MongoDB, Cassandra, and Redis for document, distributed, and cache-heavy system patterns.
6. Use the Redis interview file for cache, session, eviction, counter, and data-structure speaking practice.
7. Use the Redis troubleshooting file for hot keys, stale cache, memory pressure, rate-limiter bugs, and replication-lag scenarios.
8. Use Solr notes when search indexing and query systems are relevant.
9. Finish with the broader interview Q&A and senior guide.
